...
myData[, paste("P", 1:37, sep="")] <- scale(1 + myData[, paste("P", 1:37, sep="")])
#myData[, paste("P", 1:37, sep="")] <- scale( myData[, paste("P", 1:37, sep="")])


#dat<-myData[, paste("P", 1:37, sep="")]


#dat<-as.data.frame( sapply(dat, function(x)
#  plogis(1 + x)))
#myData[, paste("P", 1:37, sep="")] <-dat

#names(myData)

myData$revenue <- log(myData$revenue)
#myData$revenue <- scale(myData$revenue)
...
#GBM
res.bestglm <-bestglm(Xy = myData[1:n.train,-c(1,2)],
          family = gaussian,
